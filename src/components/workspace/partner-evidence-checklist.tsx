...
export function PartnerEvidenceChecklist({ caseId, caseStatus, evidence }: { caseId: string; caseStatus: string; evidence: Evidence[] }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-4 rounded-2xl border border-gold-500/20 bg-gold-500/[0.045] p-5 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      {STEPS.map((step) => {
        ...
        return <section className={cn("relative overflow-hidden rounded-2xl border bg-white p-5 transition-colors", state === "ACCEPTED" ? "border-leaf-200" : state === "REJECTED" ? "border-rose-200" : "border-black/[0.08]")} key={step.key}>
          ...
          {matching.length ? <div className="mt-3 space-y-2">{matching.map((item) => <a className="flex items-center gap-2 rounded-lg border border-black/[0.07] px-3 py-2 text-[11px] hover:border-gold-500/30" href={`/api/evidence/${item.id}`} key={item.id}><span className="min-w-0 flex-1 truncate font-medium text-slate-700">{item.title}</span><span className="text-slate-400">{formatBytes(item.byteSize)}</span><span className={cn("font-semibold", item.status === "ACCEPTED" ? "text-leaf-700" : item.status === "REJECTED" ? "text-rose-600" : "text-blue-700")}>{item.status === "PENDING" ? "Reviewing" : item.status.charAt(0) + item.status.slice(1).toLowerCase()}</span></a>)}</div> : null}
    ...
    {submitted === 4 ? <div className="rounded-xl border border-leaf-200 bg-leaf-50 p-4 text-sm text-leaf-700"><span className="font-semibold">Evidence set submitted.</span> The INRP2P review team will complete the checks and update your Trust Passport.</div> : null}
