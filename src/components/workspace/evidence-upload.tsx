...
export function EvidenceUpload({ caseId }: { caseId: string }) {
  ...
  return <form action={upload} className="space-y-3"><div className="grid gap-3 sm:grid-cols-2"><input className="input" name="title" placeholder="Document title" required /><select className="input" name="kind" defaultValue="IDENTITY_DOCUMENT"><option value="IDENTITY_DOCUMENT">Identity document</option><option value="PAN">PAN</option><option value="ADDRESS">Address proof</option><option value="BANK_PROOF">Bank proof</option><option value="WALLET_REPORT">Wallet report</option><option value="SOURCE_OF_FUNDS">Source of funds</option><option value="INCORPORATION">Incorporation</option><option value="AML_POLICY">AML policy</option><option value="REFERENCE">Reference</option><option value="OTHER">Other</option></select></div><input className="input" name="file" type="file" accept="application/pdf,image/jpeg,image/png" required /><button className="btn btn-gold btn-sm w-full sm:w-auto" disabled={busy}>{busy ? "Encrypting and uploading…" : "Upload restricted evidence"}</button>{message ? <p className="text-xs text-slate-600">{message}</p> : null}<p className="text-[11px] leading-relaxed text-slate-400">PDF/JPEG/PNG, 10 MB maximum. Objects are private, KMS-encrypted and downloaded through short-lived links.</p></form>;
}
