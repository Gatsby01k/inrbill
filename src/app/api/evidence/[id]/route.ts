...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { const response = NextResponse.redirect(presignEvidenceDownload(artifact.storageKey, `${artifact.title}.${extensions[artifact.mimeType] ?? "bin"}`)); response.headers.set("Cache-Control", "private, no-store"); response.headers.set("Referrer-Policy", "no-referrer"); return response; } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Vault unavailable" }, { status: 503 }); }
}
